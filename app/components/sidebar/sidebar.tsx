...
export default function Sidebar() {
    ...
    return (
      <aside className={`flex flex-col ${isOpen ? 'w-64' : 'w-16'} min-h-screen border-r border-gray-200 bg-gray-50 py-6 px-4 space-y-4 `}>
        ...
        <nav className="flex flex-col w-full space-y-3">
          {Icons.map((item) => (
            ...
            >
              ...
              <span className={isOpen ? '' : 'hidden'}>{item.objet}</span>
            </button>
          ))}
            <div className="flex absolute bottom-4 w-full rounded py-4 px-2">
              <div className="flex justify-center items-center">
                 <div className=" rounded bg-gray-300 h-7 w-7"></div>
                <span className={`text-muted-foreground text-sm ml-3 ${isOpen ? '' : 'hidden'}`}>
                  Name
                </span>
                <Ellipsis size={20} className={`ml-auto ${isOpen ? '' : 'ml-2'}`}/>
                </div>
             </div>
        </nav>
      </aside>
